...
function main(registration) { //[1, "U$MichaelU$P@$asdqwe123P@$", "U$NameU$P@$PasswordP@$", "U$UserU$P@$ad2P@$"]
    let validReg = [];
    let number = registration.shift();
    let n = Number(number);
    let regComplete = 0;
    ...
    for (i = 0; i < n; i++) {
        if ((validLogs = matchThisRgx.exec(registration[i])) !== null) {
            console.log('Registration was successful');
            validReg.push(validLogs[i]); //['U$MichaelU$P@$asdqwe123P@$']
            userCredList = registration[i].split('U$P@$');
