...
function main(numSteps, numDancers, numDays) {
	//distribute steps between dancers ==> steps/ dancers
	...
	if (percentOfStepsPerDay <= 13) {
		...
		console.log(`Yes, they will succeed in that goal! ${percentStepsPerDancer.toFixed(2)}%.`);
	} else {
		//else
		...
		console.log(`No, they will not succeed in that goal! Required ${percentStepsPerDancer.toFixed(2)}% steps to be learned per day.`);
	}
